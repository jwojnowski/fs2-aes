...
class AesPropertyTest extends CatsEffectSuite with ScalaCheckEffectSuite {
  ...
  override def munitIOTimeout: Duration = 240.seconds
  ...
  val secretKeyGen: Gen[SecretKey] = Gen.stringOfN(64, Gen.hexChar).map(SecretKey.fromHex(_).get)
  ...
  test("Encrypted data can't be decrypted using another key") {
    val wrongKey: SecretKey = SecretKey.fromHex("c0e5c54c2a40c95b40d6e837a9c147d4cd7cadeccc555e679efed48f726a5fe5").get
